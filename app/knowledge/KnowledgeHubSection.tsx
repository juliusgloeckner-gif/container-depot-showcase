...
export function KnowledgeHubSection({ vertical }: { vertical: string }) {
  ...
  return <section className="knowledge-hub knowledge-hub-compact section" id={`${config.key}-guides`}><div className="wrap">
    ...
    <div className="knowledge-feature-grid knowledge-feature-grid-visual">{featured.map((guide, index) => <Link className="knowledge-feature-card" href={`/${config.key}/guides/${guide.slug}`} key={guide.slug}><figure><OptimizedImage src={config.featureImages[index]} alt={config.featureAlts[index]} sizes="(max-width: 800px) 100vw, 33vw" /></figure><div><span>{guide.category.replaceAll("-", " ")}</span><h3>{guide.navTitle}</h3><p>{guide.description}</p><b>Open field guide →</b></div></Link>)}</div>
    <div className="knowledge-proof-row"><div><strong>32</strong><span>field guides</span></div><div><strong>100</strong><span>direct answers</span></div><div><strong>1</strong><span>size education center</span></div><div><strong>4</strong><span>downloadable reports</span></div><div className="knowledge-proof-actions"><Link className="button primary" href={`/${config.key}/resources`}>Explore resource center</Link><a className="button outline-dark" href="#quote-form">Get a free quote</a></div></div>
  </div></section>;
}
